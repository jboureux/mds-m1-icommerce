...
export const createProduct = async (name: string, slug: string, defaultPrice: number, categories: number)  => {
    try {
...
export const getAllProducts= async (categoryFilter?: number) => {
    try {
        const products = await prisma.product.findMany({
            where: categoryFilter ? { categories: { some: {categoryId: categoryFilter } }} : {},
        ...
        try {
            const updatedProduct = await prisma.$transaction(async (prisma) => {
                const product = await prisma.product.update({
                    where: { id },
                    data: { name: data.name, slug: data.slug, defaultPrice: data.defaultPrice },
                })
